...
class ZOD(Dataset):
    ...
    def __getitem__(self, index):
        if self.is_test:
            return self.load_BEV_only(index)
        else:
            if self.mosaic:
                rgb_map, targets = self.load_BEV_mosaic(index)
    ...
    def load_BEV_only(self, index):
        """Load only RGB BEV map for the testing phase"""
    ...
    def load_BEV_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            img, targets = self.load_BEV_with_targets(index)
